    ...
    // XXXX DO NOT ADD FURHTER BASES WITH STATE VARS HERE XXXX
{
    ...
    function _adjustCurrentPool(
        ...
        uint256 reinvest1;
        {
            ...
            (uint256 adminBalance0, uint256 adminBalance1) =
                _withdraw(_currentLowerTick, _currentUpperTick, _liquidity);

            reinvest0 = _paymentToken == address(token0)
                ? token0.balanceOf(address(this)) - adminBalance0 - _feeAmount
                : token0.balanceOf(address(this)) - adminBalance0;

            reinvest1 = _paymentToken == address(token1)
                ? token1.balanceOf(address(this)) - adminBalance1 - _feeAmount
                : token1.balanceOf(address(this)) - adminBalance1;
        }
    ...
    function _withdraw(
        ...
        amount0 =
            _adminBalanceToken0 +
            ((amountFeesAccrued0 * _adminFeeBPS) / 10000);
        amount1 =
            _adminBalanceToken1 +
            ((amountFeesAccrued1 * _adminFeeBPS) / 10000);
        _adminBalanceToken0 = amount0;
        _adminBalanceToken1 = amount1;
    }
