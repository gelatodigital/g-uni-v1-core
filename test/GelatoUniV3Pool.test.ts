  ...
  describe("GelatoUniV3Pool", function () {
    ...
    describe("update accepted parameters", function () {
      it("should fail if not called by owner", async function () {
        ...
        await expect(gelatoUniV3Pool.connect(gelato).updateMinTickDeviation(60))
          .to.be.reverted;

        await expect(
          gelatoUniV3Pool.connect(gelato).updateMaxTickDeviation(60000)
        ).to.be.reverted;
    ...
    describe("with liquidity deposited", function () {
      ...
      describe("simulate price moves and deposits, prove all value is returned on burn", function () {
        it("does not get tokens stuck in contract", async function () {
          ...
          await token0
            .connect(user2)
            .approve(gelatoUniV3Pool.address, "1000000000000000");
          await token1
            .connect(user2)
            .approve(gelatoUniV3Pool.address, "1000000000000000");
      ...
      describe("new admin functions", function () {
        it("test new admin functions", async function () {
          ...
          await gelatoUniV3Pool
            .connect(gelato)
            .autoWithdrawAdminBalance(10, token0.address);
          ...
          let adminBalCheck0 = await gelatoUniV3Pool.adminBalanceToken0();
          let adminBalCheck1 = await gelatoUniV3Pool.adminBalanceToken1();

          expect(adminBalCheck0).to.equal(ethers.constants.Zero);
          expect(adminBalCheck1).to.equal(ethers.constants.Zero);

          await gelatoUniV3Pool.withdrawAdminBalance();
          ...
          await gelatoUniV3Pool.connect(user0).updateHeartbeat("0");
          ...
          adminBalCheck0 = await gelatoUniV3Pool.adminBalanceToken0();
          adminBalCheck1 = await gelatoUniV3Pool.adminBalanceToken1();

          expect(adminBalCheck0).to.be.gt(ethers.constants.Zero);
          expect(adminBalCheck1).to.be.gt(ethers.constants.Zero);

          await gelatoUniV3Pool.connect(user2).withdrawAdminBalance();
